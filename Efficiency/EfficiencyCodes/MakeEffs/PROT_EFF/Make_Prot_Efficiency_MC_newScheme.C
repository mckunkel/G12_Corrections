...
void Make_Prot_Efficiency_MC_newScheme(){
  
  gStyle->SetPaintTextFormat("4.2f");
  
  TChain *ch1 = new TChain("Lep");
  //ch1->Add("/volatile/clas/clasg12/mkunkel/EFFICIENCY/ROOT/DATA/PROT_EFF/ALL_G12_Prot_Efficiency*");
  ch1->Add("/volatile/clas/clasg12/mkunkel/EFFICIENCY/ROOT/MC/PROT_EFF/ALL_G12_Prot_Efficiency*");
  
  Double_t P_p, P_phi, P_theta, Pull_Prob;
  Int_t nProt;
  ch1->SetBranchAddress("P_p",&P_p);
  ch1->SetBranchAddress("P_phi",&P_phi);
  ch1->SetBranchAddress("P_theta",&P_theta);
  ch1->SetBranchAddress("Pull_Prob",&Pull_Prob);
  ch1->SetBranchAddress("nProt",&nProt);
  
  Double_t P_Ptot_missing_fit, P_theta_missing_fit, P_phi_missing_fit;
  
  ch1->SetBranchAddress("P_Ptot_missing_fit",&P_Ptot_missing_fit);
  ch1->SetBranchAddress("P_theta_missing_fit",&P_theta_missing_fit);
  ch1->SetBranchAddress("P_phi_missing_fit",&P_phi_missing_fit);
  
  Double_t Pim_vz, Pim_vx, Pim_vy, Pip_vz, Pip_vx, Pip_vy;
  ...
  Double_t Pim_Ptot_fit, Pim_px_fit, Pim_py_fit, Pim_pz_fit, Pim_phi_fit, Pim_theta_fit, Pip_Ptot_fit, Pip_px_fit, Pip_py_fit, Pip_pz_fit, Pip_phi_fit ,Pip_theta_fit;
  ...
  TLorentzVector vPim, vPip;
  TVector3 vertex3_Pim, vertex3_Pip;
  ...
  for( Int_t i = 0; i < nEvent ; i++){
    ...
    if(Pull_Prob>0.01){
      ...
      int P_fit_fidpass = 0;
      if(clas::g12::g12_PosParticle_fiducial_cuts(P_Ptot_missing_fit, P_theta_missing_fit, P_phi_missing_fit,"nominal")){P_fit_fidpass =1;}
      
      if (Pim_fidpass ==0 || Pip_fidpass ==0 || P_fit_fidpass ==0 || Pip_tofKO == 0 || Pim_tofKO == 0) {
        Nfail++;
        continue;
      ...
      Int_t PipPimStat; Double_t PipPimDist;
      TVector3 PipPimMid = MatchTracks(vertex3_Pip, vPip.Vect(), vertex3_Pim, vPim.Vect(), &PipPimDist, &PipPimStat);
      if (PipPimStat == -1) {
        continue;
      }
      //Double_t PPip_Midx = PPipMid.X();
      //Double_t PPip_Midy = PPipMid.Y();
      Double_t PipPim_Midz = PipPimMid.Z();
      
      hzvert->Fill(PipPim_Midz);
      
      if (PipPim_Midz <= -105. && PipPim_Midz > -110. ){
        
        if (P_Ptot_missing_fit <= 0.5 && P_Ptot_missing_fit > 0. ) {
          hmissing_I_I->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_I_I->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 0.75 && P_Ptot_missing_fit > 0.5 ) {
          hmissing_I_II->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_I_II->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1. && P_Ptot_missing_fit > 0.75 ) {
          hmissing_I_III->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_I_III->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1.5 && P_Ptot_missing_fit > 1. ) {
          hmissing_I_IV->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_I_IV->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2. && P_Ptot_missing_fit > 1.5 ) {
          hmissing_I_V->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_I_V->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2.5 && P_Ptot_missing_fit > 2. ) {
          hmissing_I_VI->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_I_VI->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 3. && P_Ptot_missing_fit > 2.5 ) {
          hmissing_I_VII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_I_VII->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 5. && P_Ptot_missing_fit > 3. ) {
          hmissing_I_VIII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_I_VIII->Fill(P_theta,P_phi);}
        }
        
      }
      
      if (PipPim_Midz <= -100. && PipPim_Midz > -105. ){
        
        if (P_Ptot_missing_fit <= 0.5 && P_Ptot_missing_fit > 0. ) {
          hmissing_II_I->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_II_I->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 0.75 && P_Ptot_missing_fit > 0.5 ) {
          hmissing_II_II->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_II_II->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1. && P_Ptot_missing_fit > 0.75 ) {
          hmissing_II_III->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_II_III->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1.5 && P_Ptot_missing_fit > 1. ) {
          hmissing_II_IV->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_II_IV->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2. && P_Ptot_missing_fit > 1.5 ) {
          hmissing_II_V->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_II_V->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2.5 && P_Ptot_missing_fit > 2. ) {
          hmissing_II_VI->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_II_VI->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 3. && P_Ptot_missing_fit > 2.5 ) {
          hmissing_II_VII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_II_VII->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 5. && P_Ptot_missing_fit > 3. ) {
          hmissing_II_VIII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_II_VIII->Fill(P_theta,P_phi);}
        }
        
      }
      
      if (PipPim_Midz <= -95. && PipPim_Midz > -100. ){
        
        if (P_Ptot_missing_fit <= 0.5 && P_Ptot_missing_fit > 0. ) {
          hmissing_III_I->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_III_I->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 0.75 && P_Ptot_missing_fit > 0.5 ) {
          hmissing_III_II->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_III_II->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1. && P_Ptot_missing_fit > 0.75 ) {
          hmissing_III_III->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_III_III->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1.5 && P_Ptot_missing_fit > 1. ) {
          hmissing_III_IV->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_III_IV->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2. && P_Ptot_missing_fit > 1.5 ) {
          hmissing_III_V->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_III_V->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2.5 && P_Ptot_missing_fit > 2. ) {
          hmissing_III_VI->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_III_VI->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 3. && P_Ptot_missing_fit > 2.5 ) {
          hmissing_III_VII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_III_VII->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 5. && P_Ptot_missing_fit > 3. ) {
          hmissing_III_VIII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_III_VIII->Fill(P_theta,P_phi);}
        }
        
      }
      
      if (PipPim_Midz <= -90. && PipPim_Midz > -95. ){
        
        if (P_Ptot_missing_fit <= 0.5 && P_Ptot_missing_fit > 0. ) {
          hmissing_IV_I->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_IV_I->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 0.75 && P_Ptot_missing_fit > 0.5 ) {
          hmissing_IV_II->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_IV_II->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1. && P_Ptot_missing_fit > 0.75 ) {
          hmissing_IV_III->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_IV_III->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1.5 && P_Ptot_missing_fit > 1. ) {
          hmissing_IV_IV->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_IV_IV->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2. && P_Ptot_missing_fit > 1.5 ) {
          hmissing_IV_V->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_IV_V->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2.5 && P_Ptot_missing_fit > 2. ) {
          hmissing_IV_VI->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_IV_VI->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 3. && P_Ptot_missing_fit > 2.5 ) {
          hmissing_IV_VII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_IV_VII->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 5. && P_Ptot_missing_fit > 3. ) {
          hmissing_IV_VIII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_IV_VIII->Fill(P_theta,P_phi);}
        }
        
      }
      
      if (PipPim_Midz <= -85. && PipPim_Midz > -90. ){
        
        if (P_Ptot_missing_fit <= 0.5 && P_Ptot_missing_fit > 0. ) {
          hmissing_V_I->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_V_I->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 0.75 && P_Ptot_missing_fit > 0.5 ) {
          hmissing_V_II->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_V_II->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1. && P_Ptot_missing_fit > 0.75 ) {
          hmissing_V_III->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_V_III->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1.5 && P_Ptot_missing_fit > 1. ) {
          hmissing_V_IV->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_V_IV->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2. && P_Ptot_missing_fit > 1.5 ) {
          hmissing_V_V->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_V_V->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2.5 && P_Ptot_missing_fit > 2. ) {
          hmissing_V_VI->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_V_VI->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 3. && P_Ptot_missing_fit > 2.5 ) {
          hmissing_V_VII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_V_VII->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 5. && P_Ptot_missing_fit > 3. ) {
          hmissing_V_VIII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_V_VIII->Fill(P_theta,P_phi);}
        }
        
      }
      
      if (PipPim_Midz <= -80. && PipPim_Midz > -85. ){
        
        if (P_Ptot_missing_fit <= 0.5 && P_Ptot_missing_fit > 0. ) {
          hmissing_VI_I->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VI_I->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 0.75 && P_Ptot_missing_fit > 0.5 ) {
          hmissing_VI_II->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VI_II->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1. && P_Ptot_missing_fit > 0.75 ) {
          hmissing_VI_III->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VI_III->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1.5 && P_Ptot_missing_fit > 1. ) {
          hmissing_VI_IV->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VI_IV->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2. && P_Ptot_missing_fit > 1.5 ) {
          hmissing_VI_V->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VI_V->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2.5 && P_Ptot_missing_fit > 2. ) {
          hmissing_VI_VI->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VI_VI->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 3. && P_Ptot_missing_fit > 2.5 ) {
          hmissing_VI_VII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VI_VII->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 5. && P_Ptot_missing_fit > 3. ) {
          hmissing_VI_VIII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VI_VIII->Fill(P_theta,P_phi);}
        }
        
      }
      
      if (PipPim_Midz <= -75. && PipPim_Midz > -80. ){
        
        if (P_Ptot_missing_fit <= 0.5 && P_Ptot_missing_fit > 0. ) {
          hmissing_VII_I->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VII_I->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 0.75 && P_Ptot_missing_fit > 0.5 ) {
          hmissing_VII_II->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VII_II->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1. && P_Ptot_missing_fit > 0.75 ) {
          hmissing_VII_III->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VII_III->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1.5 && P_Ptot_missing_fit > 1. ) {
          hmissing_VII_IV->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VII_IV->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2. && P_Ptot_missing_fit > 1.5 ) {
          hmissing_VII_V->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VII_V->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2.5 && P_Ptot_missing_fit > 2. ) {
          hmissing_VII_VI->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VII_VI->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 3. && P_Ptot_missing_fit > 2.5 ) {
          hmissing_VII_VII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VII_VII->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 5. && P_Ptot_missing_fit > 3. ) {
          hmissing_VII_VIII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VII_VIII->Fill(P_theta,P_phi);}
        }
        
      }
      
      if (PipPim_Midz <= -70. && PipPim_Midz > -75. ){
        
        if (P_Ptot_missing_fit <= 0.5 && P_Ptot_missing_fit > 0. ) {
          hmissing_VIII_I->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VIII_I->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 0.75 && P_Ptot_missing_fit > 0.5 ) {
          hmissing_VIII_II->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VIII_II->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1. && P_Ptot_missing_fit > 0.75 ) {
          hmissing_VIII_III->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VIII_III->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 1.5 && P_Ptot_missing_fit > 1. ) {
          hmissing_VIII_IV->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VIII_IV->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2. && P_Ptot_missing_fit > 1.5 ) {
          hmissing_VIII_V->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VIII_V->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 2.5 && P_Ptot_missing_fit > 2. ) {
          hmissing_VIII_VI->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VIII_VI->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 3. && P_Ptot_missing_fit > 2.5 ) {
          hmissing_VIII_VII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VIII_VII->Fill(P_theta,P_phi);}
        }
        
        if (P_Ptot_missing_fit <= 5. && P_Ptot_missing_fit > 3. ) {
          hmissing_VIII_VIII->Fill(P_theta_missing_fit,P_phi_missing_fit);
          if (nProt== 1 && P_fidpass== 1 && P_tofKO  == 1 ) {hdetect_VIII_VIII->Fill(P_theta,P_phi);}
        }
      ...
      hfit_p->Fill(P_Ptot_missing_fit);
      hfit_phi->Fill(P_phi_missing_fit);
      hfit_theta->Fill(P_theta_missing_fit);
      
      if (nProt ==1 && P_fidpass==1) {
        hdetect_p->Fill(P_p);
        hdetect_phi->Fill(P_phi);
        hdetect_theta->Fill(P_theta);
