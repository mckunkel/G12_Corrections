...
void Make_Prot_EfficiencyTH3D_MC(){
  
  gStyle->SetPaintTextFormat("4.2f");
  
  TChain *ch1 = new TChain("Lep");
  //ch1->Add("/volatile/clas/clasg12/mkunkel/EFFICIENCY/ROOT/DATA/PROT_EFF/ALL_HADDED/ALL_g12*.root");
  ch1->Add("/volatile/clas/clasg12/mkunkel/EFFICIENCY/ROOT/MC/PROT_EFF/ALL_G12_Prot_Efficiency*");
  
  ch1->SetBranchStatus("*",0);
  ch1->SetBranchStatus("P_p",1);
  ch1->SetBranchStatus("P_phi",1);
  ch1->SetBranchStatus("P_theta",1);
  ch1->SetBranchStatus("Pull_Prob",1);
  ch1->SetBranchStatus("nProt",1);
  
  ch1->SetBranchStatus("P_Ptot_missing_fit",1);
  ch1->SetBranchStatus("P_theta_missing_fit",1);
  ch1->SetBranchStatus("P_phi_missing_fit",1);
  ...
  for( Int_t i = 0; i < nEvent ; i++){
    ...
    if(Pull_Prob>0.01){
      ...
      Npass++;
      hmissing->Fill(P_Ptot_missing_fit, P_theta_missing_fit, P_phi_missing_fit);
      if (nProt == 1 && P_fidpass == 1 && P_tofKO  == 1) {hdetect->Fill(P_p, P_theta,P_phi);}
  ...
  TFile fout("Efficiency_Plots3D_MC_Prot.root","recreate");
