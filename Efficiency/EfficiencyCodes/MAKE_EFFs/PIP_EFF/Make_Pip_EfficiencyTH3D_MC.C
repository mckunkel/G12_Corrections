...
void Make_Pip_EfficiencyTH3D_MC(){
  
  gStyle->SetPaintTextFormat("4.2f");
  
  TChain *ch1 = new TChain("Lep");
  //ch1->Add("/volatile/clas/clasg12/mkunkel/EFFICIENCY/ROOT/DATA/PIP_EFF/ALL_HADDED/ALL_g12*.root");
  ch1->Add("/volatile/clas/clasg12/mkunkel/EFFICIENCY/ROOT/MC/PIP_EFF/ALL_G12_Pip_Efficiency*");
  
  ch1->SetBranchStatus("*",0);
  ch1->SetBranchStatus("Pip_p",1);
  ch1->SetBranchStatus("Pip_phi",1);
  ch1->SetBranchStatus("Pip_theta",1);
  ch1->SetBranchStatus("Pull_Prob",1);
  ch1->SetBranchStatus("nPip",1);
  
  ch1->SetBranchStatus("Pip_Ptot_missing_fit",1);
  ch1->SetBranchStatus("Pip_theta_missing_fit",1);
  ch1->SetBranchStatus("Pip_phi_missing_fit",1);
  ...
  for( Int_t i = 0; i < nEvent ; i++){
    ...
    if(Pull_Prob>0.01){
      ...
      hmissing->Fill(Pip_Ptot_missing_fit, Pip_theta_missing_fit, Pip_phi_missing_fit);
      if (nPip == 1 && Pip_fidpass == 1 && Pip_tofKO  == 1) {hdetect->Fill(Pip_p, Pip_theta,Pip_phi);}
  ...
  TFile fout("Efficiency_Plots3D_MC_Pip.root","recreate");
