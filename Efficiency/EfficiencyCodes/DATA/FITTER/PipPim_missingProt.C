...
int main(int argc, char* argv[]){
  ...
  string parms_dir = "/work/clas/clasg12/mkunkel/twolep/NOPID/EFFICIENCY/SUBMIT_ALL_EFF/FITTER/KINFIT_CODES/parms";
  
  // Initialization of Momentum Correction Parameters
  
  //        clog << "Reading in momentum correction parameters..."<<endl;
  ...
  Double_t P_px_missing_fit, P_py_missing_fit, P_pz_missing_fit, P_theta_missing_fit, P_phi_missing_fit;
  
  t4->Branch("P_px_missing_fit",&P_px_missing_fit,"P_px_missing_fit/D");
  t4->Branch("P_py_missing_fit",&P_py_missing_fit,"P_py_missing_fit/D");
  t4->Branch("P_pz_missing_fit",&P_pz_missing_fit,"P_pz_missing_fit/D");
  t4->Branch("P_theta_missing_fit",&P_theta_missing_fit,"P_theta_missing_fit/D");
  t4->Branch("P_phi_missing_fit",&P_phi_missing_fit,"P_phi_missing_fit/D");
  
  Double_t P_px_missing, P_py_missing, P_pz_missing, P_theta_missing, P_phi_missing;
  t4->Branch("P_px_missing",&P_px_missing,"P_px_missing/D");
  t4->Branch("P_py_missing",&P_py_missing,"P_py_missing/D");
  t4->Branch("P_pz_missing",&P_pz_missing,"P_pz_missing/D");
  t4->Branch("P_theta_missing",&P_theta_missing,"P_theta_missing/D");
  t4->Branch("P_phi_missing",&P_phi_missing,"P_phi_missing/D");
  
  
  Double_t P_Ptot_missing, P_Ptot_missing_fit;
  
  t4->Branch("P_Ptot_missing",&P_Ptot_missing,"P_Ptot_missing/D");
  t4->Branch("P_Ptot_missing_fit",&P_Ptot_missing_fit,"P_Ptot_missing_fit/D");
  ...
  //--------------------------------------------------- Both PiPlus and PiMinus---------------------------------------------------------
  
  Double_t Pip_Ptot_fit, Pim_Ptot_fit;
  
  t4->Branch("Pip_Ptot_fit",&Pip_Ptot_fit,"Pip_Ptot_fit/D");
  t4->Branch("Pim_Ptot_fit",&Pim_Ptot_fit,"Pim_Ptot_fit/D");
  ...
  Double_t mm_PipPim_fit, mm_PipPim, mm2_PipPim_fit, mm2_PipPim;
  t4->Branch("mm_PipPim_fit",&mm_PipPim_fit,"mm_PipPim_fit/D");
  t4->Branch("mm_PipPim",&mm_PipPim,"mm_PipPim/D");
  t4->Branch("mm2_PipPim_fit",&mm2_PipPim_fit,"mm2_PipPim_fit/D");
  t4->Branch("mm2_PipPim",&mm2_PipPim,"mm2_PipPim/D");
  ...
  TLorentzVector vT, vEg, vP, vPip, vPim, vPip_fit, vPim_fit, vEg_fit;
  ...
  for( Int_t i = 0; i < nEvent ; i++){
    ...
    Eg = clas::g12::corrected_beam_energy(run_input, egam);
    ...
    TLorentzVector MM_PipPimVec = (vEg + vT) - (vPip + vPim);
    mm2_PipPim = MM_PipPimVec.M2();
    mm_PipPim = MM_PipPimVec.M();
    
    P_px_missing = MM_PipPimVec.Px();
    P_py_missing = MM_PipPimVec.Py();
    P_pz_missing = MM_PipPimVec.Pz();
    P_theta_missing = RadtoDeg*MM_PipPimVec.Theta();
    P_phi_missing = RadtoDeg*MM_PipPimVec.Phi();
    
    P_Ptot_missing = sqrt(pow(P_px_missing,2) + pow(P_py_missing,2) + pow(P_pz_missing,2));
    ...
    //    covTrack(1,1)= prot_mat[0][0]*pow(P_Ptot,4);covTrack(1,2)= -prot_mat[0][1]*pow(P_Ptot,2);covTrack(1,3)= -prot_mat[0][2]*pow(P_Ptot,2);
    //    covTrack(2,1)= -prot_mat[0][1]*pow(P_Ptot,2);covTrack(2,2)= prot_mat[1][1];covTrack(2,3)= prot_mat[1][2];
    //    covTrack(3,1)= -prot_mat[0][2]*pow(P_Ptot,2);covTrack(3,2)= prot_mat[1][2];covTrack(3,3)=prot_mat[2][2];
    //
    //    //----------------------------------------------------For Pip-------------------------------------------------------------------
    //
    ...
    covTrack(1,1)= pip_mat[0][0]*pow(Pip_p,4);covTrack(1,2)= -pip_mat[0][1]*pow(Pip_p,2);covTrack(1,3)= -pip_mat[0][2]*pow(Pip_p,2);
    covTrack(2,1)= -pip_mat[0][1]*pow(Pip_p,2);covTrack(2,2)= pip_mat[1][1];covTrack(2,3)= pim_mat[1][2];
    covTrack(3,1)= -pip_mat[0][2]*pow(Pip_p,2);covTrack(3,2)= pip_mat[1][2];covTrack(3,3)= pip_mat[2][2];
    
    //----------------------------------------------------For Pim-------------------------------------------------------------------
    covTrack(4,4)= pim_mat[0][0]*pow(Pim_p,4);covTrack(4,5)= pim_mat[0][1]*pow(Pim_p,2);covTrack(4,6)= pim_mat[0][2]*pow(Pim_p,2);
    covTrack(5,4)= pim_mat[0][1]*pow(Pim_p,2);covTrack(5,5)= pim_mat[1][1];covTrack(5,6)= pim_mat[1][2];
    covTrack(6,4)= pim_mat[0][2]*pow(Pim_p,2);covTrack(6,5)= pim_mat[1][2];covTrack(6,6)= pim_mat[2][2];
    ...
    TLorentzVector MM_PipPimVec_fit = vEg_fit + vT - (vPip_fit + vPim_fit);
    
    mm_PipPim_fit = MM_PipPimVec_fit.M();
    mm2_PipPim_fit = MM_PipPimVec_fit.M2();
    
    //    P_px_fit = vP.Px();
    //    P_py_fit = vP.Py();
    //    P_pz_fit = vP.Pz();
    
    P_px_missing_fit = MM_PipPimVec_fit.Px();
    P_py_missing_fit = MM_PipPimVec_fit.Py();
    P_pz_missing_fit = MM_PipPimVec_fit.Pz();
    P_theta_missing_fit = RadtoDeg*MM_PipPimVec_fit.Theta();
    P_phi_missing_fit = RadtoDeg*MM_PipPimVec_fit.Phi();
    ...
    Pim_px_fit = vPim_fit.Px();
    Pim_py_fit = vPim_fit.Py();
    Pim_pz_fit = vPim_fit.Pz();
    Pim_theta_fit = RadtoDeg*vPim_fit.Theta();
    Pim_phi_fit = RadtoDeg*vPim_fit.Phi();
    
    
    P_Ptot_missing_fit = sqrt(pow(P_px_missing_fit,2) + pow(P_py_missing_fit,2) + pow(P_pz_missing_fit,2));
    Pip_Ptot_fit = sqrt(pow(Pip_px_fit,2) + pow(Pip_py_fit,2) + pow(Pip_pz_fit,2));
    Pim_Ptot_fit = sqrt(pow(Pim_px_fit,2) + pow(Pim_py_fit,2) + pow(Pim_pz_fit,2));
    
    
    if(Pull_Prob>0.01 && Pip_fidpass ==1 && Pim_fidpass ==1){
      t4->Fill();
    }
    // t4 ->Fill();
