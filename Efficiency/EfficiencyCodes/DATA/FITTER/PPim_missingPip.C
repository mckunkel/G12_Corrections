...
int main(int argc, char* argv[]){
  ...
  Double_t Pip_px_missing_fit, Pip_py_missing_fit, Pip_pz_missing_fit, Pip_theta_missing_fit, Pip_phi_missing_fit;
  
  t4->Branch("Pip_px_missing_fit",&Pip_px_missing_fit,"Pip_px_missing_fit/D");
  t4->Branch("Pip_py_missing_fit",&Pip_py_missing_fit,"Pip_py_missing_fit/D");
  t4->Branch("Pip_pz_missing_fit",&Pip_pz_missing_fit,"Pip_pz_missing_fit/D");
  t4->Branch("Pip_theta_missing_fit",&Pip_theta_missing_fit,"Pip_theta_missing_fit/D");
  t4->Branch("Pip_phi_missing_fit",&Pip_phi_missing_fit,"Pip_phi_missing_fit/D");
  
  Double_t Pip_px_missing, Pip_py_missing, Pip_pz_missing, Pip_theta_missing, Pip_phi_missing;
  t4->Branch("Pip_px_missing",&Pip_px_missing,"Pip_px_missing/D");
  t4->Branch("Pip_py_missing",&Pip_py_missing,"Pip_py_missing/D");
  t4->Branch("Pip_pz_missing",&Pip_pz_missing,"Pip_pz_missing/D");
  t4->Branch("Pip_theta_missing",&Pip_theta_missing,"Pip_theta_missing/D");
  t4->Branch("Pip_phi_missing",&Pip_phi_missing,"Pip_phi_missing/D");
  
  
  Double_t Pip_Ptot_missing, Pip_Ptot_missing_fit;
  
  t4->Branch("Pip_Ptot_missing",&Pip_Ptot_missing,"Pip_Ptot_missing/D");
  t4->Branch("Pip_Ptot_missing_fit",&Pip_Ptot_missing_fit,"Pip_Ptot_missing_fit/D");
  ...
  Double_t mm_PPim_fit, mm_PPim, mm2_PPim_fit, mm2_PPim;
  t4->Branch("mm_PPim_fit",&mm_PPim_fit,"mm_PPim_fit/D");
  t4->Branch("mm_PPim",&mm_PPim,"mm_PPim/D");
  t4->Branch("mm2_PPim_fit",&mm2_PPim_fit,"mm2_PPim_fit/D");
  t4->Branch("mm2_PPim",&mm2_PPim,"mm2_PPim/D");
  ...
  TLorentzVector vT, vEg, vP, vPip, vPim, vPim_fit, vP_fit, vEg_fit;
  ...
  for( Int_t i = 0; i < nEvent ; i++){
    ...
    TLorentzVector MM_PPimVec = (vEg + vT) - (vP + vPim);
    mm2_PPim = MM_PPimVec.M2();
    mm_PPim = MM_PPimVec.M();
    
    Pip_px_missing = MM_PPimVec.Px();
    Pip_py_missing = MM_PPimVec.Py();
    Pip_pz_missing = MM_PPimVec.Pz();
    Pip_theta_missing = RadtoDeg*MM_PPimVec.Theta();
    Pip_phi_missing = RadtoDeg*MM_PPimVec.Phi();
    
    Pip_Ptot_missing = sqrt(pow(Pip_px_missing,2) + pow(Pip_py_missing,2) + pow(Pip_pz_missing,2));
    ...
    TLorentzVector MM_PPimVec_fit = vEg_fit + vT - (vP_fit + vPim_fit);
    
    mm_PPim_fit = MM_PPimVec_fit.M();
    mm2_PPim_fit = MM_PPimVec_fit.M2();
    ...
    Pip_px_missing_fit = MM_PPimVec_fit.Px();
    Pip_py_missing_fit = MM_PPimVec_fit.Py();
    Pip_pz_missing_fit = MM_PPimVec_fit.Pz();
    Pip_theta_missing_fit = RadtoDeg*MM_PPimVec_fit.Theta();
    Pip_phi_missing_fit = RadtoDeg*MM_PPimVec_fit.Phi();
    ...
    Pip_Ptot_missing_fit = sqrt(pow(Pip_px_missing_fit,2) + pow(Pip_py_missing_fit,2) + pow(Pip_pz_missing_fit,2));
    
    
    if(Pull_Prob>0.01 && P_fidpass ==1 && Pim_fidpass ==1){
      t4->Fill();
    }
    // t4 ->Fill();
  }
